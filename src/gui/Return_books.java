    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) throws IOException {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        for(students s : students) {
        	if(s.getId() == Integer.parseInt(id)) {
        		for(Books x :books) {
        			if(call.equalsIgnoreCase(x.getCallno())) {
        				if(s.has_book(x)) {
            				x.setQuantity(x.getQuantity()+1);
            				x.setIssued(x.getIssued()-1);
            				s.return_book(x);
            				file_manger.save_books(books);
            				file_manger.save_students(students);
            		    	this.hide();
            		        JOptionPane.showMessageDialog(null,"Book returned successfully");
            		        Librarian_section f= new Librarian_section();
            		        f.setVisible(true);
            				break;
        				}
        				else
        					JOptionPane.showMessageDialog(null,"This student didn't issue the book");
